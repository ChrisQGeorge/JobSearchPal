...
9. **Preferences & Identity** — Dedicated page for everything about *who the user is* and *what they want in a job* that is distinct from their achieved history. Organized into three clearly separated panels:
...
18. **JobPreferences** — Singleton per user. Captures the user's job-search criteria as scalar values across three tiers (*preferred*, *acceptable*, *unacceptable* where applicable). id, user_id, salary_currency (ISO 4217, e.g., USD), salary_preferred_target, salary_acceptable_min, salary_unacceptable_below, total_comp_preferred_target, total_comp_acceptable_min, experience_level_preferred (intern / junior / mid / senior / staff / principal / manager / director / vp / cxo), experience_levels_acceptable *(JSON array)*, experience_levels_unacceptable *(JSON array)*, remote_policy_preferred (remote / hybrid / onsite), remote_policies_acceptable *(JSON array)*, remote_policies_unacceptable *(JSON array)*, max_commute_minutes_preferred, max_commute_minutes_acceptable, willing_to_relocate (bool), relocation_notes, travel_percent_preferred, travel_percent_acceptable_max, travel_percent_unacceptable_above, hours_per_week_preferred, hours_per_week_acceptable_max, overtime_acceptable (bool), schedule_preferred (standard / flexible / async / shift / other), schedules_acceptable *(JSON array)*, schedules_unacceptable *(JSON array)*, employment_types_preferred *(JSON array: full_time / part_time / contract / c2h / internship / freelance)*, employment_types_acceptable *(JSON array)*, employment_types_unacceptable *(JSON array)*, equity_preference (required / preferred / nice_to_have / not_important / unwanted), benefits_required *(JSON array: health / dental / vision / 401k_match / equity / pto / parental_leave / remote_stipend / etc.)*, benefits_preferred *(JSON array)*, earliest_start_date, latest_start_date, notice_period_weeks, dealbreakers_notes (free text — strong no's), dream_job_notes (free text — strong yes's), updated_at.
19. **JobCriterion** — Generic tiered criterion table for list-style preferences that are more naturally many-valued than scalar. id, user_id, category (industry / role_title / technology / company_size / company_type / mission_area / location / language / benefit / management_style / team_size / other), value (string — e.g., "healthcare", "React", "11-50", "nonprofit", "climate"), tier (preferred / acceptable / unacceptable), weight (1–5, optional — higher means stronger match/mismatch signal), notes. Composite unique key: (user_id, category, value). Semantic interpretation: *preferred* the user actively wants, *acceptable* the user will take if offered, *unacceptable* is a dealbreaker and should flag the job.
20. **WorkAuthorization** — Singleton per user. id, user_id, current_country, current_location_city, current_location_region, citizenship_countries *(JSON array)*, work_authorization_status (citizen / permanent_resident / work_visa / student_visa / dependent_visa / asylum_refugee / temporary_protected_status / need_sponsorship / other), visa_type (nullable — H-1B / L-1 / TN / O-1 / OPT / CPT / EAD / etc.), visa_issued_date (nullable), visa_expires_date (nullable), visa_sponsorship_required_now (bool), visa_sponsorship_required_future (bool), relocation_countries_acceptable *(JSON array)*, security_clearance_level (none / public_trust / confidential / secret / top_secret / ts_sci / ts_sci_with_poly / other), security_clearance_active (bool), security_clearance_notes, export_control_considerations (nullable — e.g., ITAR-restricted background), updated_at.
21. **Demographics** — Singleton per user. Voluntary EEOC-style self-identification data. **Every field is optional**; an unset field is indistinguishable from "prefer not to say" externally. id, user_id, preferred_name, legal_first_name, legal_middle_name, legal_last_name, legal_suffix, pronouns (he_him / she_her / they_them / self_describe / prefer_not_to_say), pronouns_self_describe (nullable), gender_identity (man / woman / non_binary / self_describe / prefer_not_to_say), gender_self_describe (nullable), sex_assigned_at_birth (male / female / intersex / prefer_not_to_say), transgender_identification (yes / no / prefer_not_to_say), sexual_orientation (straight / gay / lesbian / bisexual / queer / asexual / self_describe / prefer_not_to_say), sexual_orientation_self_describe (nullable), race_ethnicity *(JSON array — multi-select: american_indian_or_alaska_native / asian / black_or_african_american / hispanic_or_latino / middle_eastern_or_north_african / native_hawaiian_or_pacific_islander / white / two_or_more / self_describe / prefer_not_to_say)*, ethnicity_self_describe (nullable), veteran_status (not_a_veteran / veteran / protected_veteran / disabled_veteran / recently_separated / active_duty_wartime_or_campaign_badge / armed_forces_service_medal / prefer_not_to_say), disability_status (yes / no / prefer_not_to_say), disability_notes (nullable — user's own description, never required), accommodation_needs (nullable — accommodations the user may request during interviewing), date_of_birth (nullable), age_bracket (under_18 / 18_24 / 25_34 / 35_44 / 45_54 / 55_64 / 65_plus / prefer_not_to_say — may be derived from DOB), first_generation_college_student (yes / no / prefer_not_to_say), updated_at.
22. **DemographicSharePolicy** — Per-field share policy governing how `application-autofiller` may use each demographic field. id, user_id, field_name (must match a column on `Demographics` — e.g., `veteran_status`, `disability_status`, `race_ethnicity`), policy (always_share / share_on_request / ask_each_time / never_share), override_value (nullable — if set, autofill returns this value instead of the canonical value; used when the user wants to always answer "prefer not to say" for a field whose true value is recorded locally), updated_at. Composite unique key: (user_id, field_name). Default policy for every new field: `ask_each_time`.

**Companies and jobs**

23. **Company** — id, name, website, industry, size (e.g., "11-50", "201-500"), headquarters_location, founded_year, description, research_notes (AI-assembled summary), source_links *(JSON)*, reputation_signals *(JSON — e.g., Glassdoor rating, news highlights)*, tech_stack_hints *(JSON)*.
24. **TrackedJob** — id, user_id, company_id (FK), title, job_description, source_url, source_platform (linkedin / indeed / company_site / referral / other), location, remote_policy (onsite / hybrid / remote), salary_min, salary_max, salary_currency, equity_notes, priority (low / medium / high), status (enum: watching / interested / applied / responded / screening / interviewing / assessment / offer / won / lost / withdrawn / ghosted / archived), notes, jd_analysis *(JSON — output of jd-analyzer skill)*, fit_summary *(JSON — output of job-fit-scorer: score, alignments, mismatches, dealbreakers)*, date_posted, date_discovered, date_applied, date_closed.
25. **ApplicationEvent** — id, tracked_job_id (FK), event_type (applied / responded / phone_screen / interview_scheduled / interview_completed / assessment_assigned / assessment_submitted / offer_received / offer_accepted / offer_declined / rejection / withdrawal / follow_up / note / other), event_date, details_md, related_round_id (FK InterviewRound nullable), attachments *(JSON array of InterviewArtifact IDs or file refs)*.
26. **InterviewRound** — id, tracked_job_id (FK), round_number, round_type (recruiter_screen / hiring_manager / technical / system_design / behavioral / panel / take_home / onsite / final / other), scheduled_at, duration_minutes, format (phone / video / in_person), location_or_link, interviewers *(JSON array of {name, role, contact_id?}*), outcome (pending / passed / failed / mixed / unknown), self_rating (1–5), notes_md, prep_notes_md.
27. **InterviewArtifact** — id, interview_round_id (FK nullable), tracked_job_id (FK), kind (take_home / whiteboard_capture / notes / feedback / offer_letter / recruiter_email / prep_doc / other), title, file_url, mime_type, content_md (for inline text artifacts), source (uploaded / generated / pasted), tags *(JSON)*.

**Generated content and Companion**

28. **GeneratedDocument** — id, user_id, tracked_job_id (FK nullable), doc_type (resume / cv / cover_letter / email / interview_prep / other), title, content_md, content_structured *(JSON — optional, for templated formats)*, version, parent_version_id (nullable, self-FK), humanized (bool), humanized_from_samples *(JSON array of WritingSample IDs)*, model_used, persona_id (FK Persona), prompt_snapshot, source_skill.
29. **DocumentEdit** — id, generated_document_id (FK), editor (user / companion), action (rewrite_selection / accept_suggestion / reject_suggestion / manual_edit), selection_start, selection_end, selection_text, user_notes, replacement_text, accepted (bool), created_at.
30. **WritingSample** — id, user_id, title, content_md, tags *(JSON)*, source (original / blog / email / paper / journal / other), word_count, style_signals *(JSON — optional cached analysis: avg sentence length, vocabulary markers, etc.)*.
31. **CompanionConversation** — id, user_id, title, summary, pinned (bool), related_tracked_job_id (FK nullable), persona_id (FK Persona).
32. **ConversationMessage** — id, conversation_id (FK), role (user / assistant / tool / system), content_md, skill_invoked (nullable), tool_calls *(JSON)*, tool_results *(JSON)*, attachments *(JSON)*, created_at.
33. **Task** — id, user_id, tracked_job_id (FK nullable), title, description, due_date, status (open / in_progress / done / dismissed), priority, source (companion / manual), related_event_id (FK ApplicationEvent nullable).

**Operational**

34. **AuditLog** — id, user_id, actor (user / skill_name / system), action, entity_type, entity_id, diff *(JSON)*, created_at. Captures every mutation so the user can review what skills changed on their behalf.
35. **AutofillLog** — id, user_id, tracked_job_id (FK nullable), fields_shared *(JSON array of {field_name, value_returned, source (canonical / override / user_prompted), policy_applied})*, recipient (manual_copy / generated_document_id / other), created_at. Records exactly what demographic / identity data was emitted by `application-autofiller` for any given invocation, for user review.
36. **MetricSnapshot** — id, user_id, metric_key, period (daily / weekly / monthly / all_time), period_start, period_end, value *(JSON)*, computed_at. Optional caching layer for dashboard charts.
...
12. **job-fit-scorer** — Scores a `TrackedJob` against the user's `JobPreferences`, `JobCriterion`, and `WorkAuthorization` records, using `TrackedJob.jd_analysis` as evidence. Produces a match percentage plus structured lists of **alignments** (job meets a preferred/required criterion), **mismatches** (job falls into acceptable but not preferred), and **dealbreakers** (job falls into unacceptable / sponsorship mismatch / salary-below-floor). Result is cached on `TrackedJob.fit_summary` and surfaced in the Job Tracker and Job Detail.
13. **application-autofiller** — Produces answers to common application-form fields (personal info, work authorization, voluntary demographics) drawn from `User`, `WorkAuthorization`, and `Demographics`. Always respects `DemographicSharePolicy`: fields set to `never_share` are omitted, `always_share` fields are filled automatically, `share_on_request` / `ask_each_time` fields are returned with a per-field confirmation prompt, and `override_value`s replace canonical values when set. The skill **does not** send demographic or authorization data to the LLM provider as free text — the backend emits named placeholders (e.g., `{{demographics.veteran_status}}`) and fills them locally after LLM processing. Every invocation writes an `AutofillLog`.
14. **selection-rewriter** — Targeted skill invoked by the in-editor "Send to Companion" action. Takes a selection, surrounding context, and user notes; returns a rewrite. Always records a `DocumentEdit`.
15. **companion-persona** — Not a task skill — the wrapper that keeps Claude Code in-character across interactions according to the active `Persona`. Other skills defer tone decisions to this one.
...
**Job preferences and fit**
- `REQ-FUNC-PREF-001` — The system **shall** support recording job criteria across three tiers (`preferred`, `acceptable`, `unacceptable`) for the scalar fields on `JobPreferences` (salary, experience level, remote policy, commute, travel, hours, schedule, employment type, equity, benefits, start date) and for the list-valued categories on `JobCriterion` (industry, role_title, technology, company_size, company_type, mission_area, location, language, benefit, management_style, team_size, other).
- `REQ-FUNC-PREF-002` — The Preferences & Identity page **shall** validate `JobPreferences` inputs: salary values **shall** be non-negative; tier thresholds **shall** be mutually consistent (e.g., `salary_unacceptable_below` ≤ `salary_acceptable_min` ≤ `salary_preferred_target`); date fields **shall** be correctly ordered. Invalid entries block save with a field-level error.
- `REQ-FUNC-PREF-003` — When a `TrackedJob` is created, updated, or has its JD analyzed, the `job-fit-scorer` skill **shall** compute a `fit_summary` comparing the job against the user's `JobPreferences`, `JobCriterion`, and `WorkAuthorization`. The summary **shall** include a numeric score, a list of alignments, a list of mismatches, and a prominent list of dealbreakers.
- `REQ-FUNC-PREF-004` — The Job Tracker **shall** expose a filter for fit score and a visible indicator when a `TrackedJob` contains any dealbreaker. The user **shall** be able to override a dealbreaker (marking it *acknowledged*) without removing it from the record.

**Work authorization**
- `REQ-FUNC-AUTH-001` — The system **shall** store the user's `WorkAuthorization` record and surface its fields on the Preferences & Identity page, with clear language distinguishing *sponsorship required now* from *sponsorship required in the future*.
- `REQ-FUNC-AUTH-002` — `job-fit-scorer` **shall** treat mismatches between the job's required work authorization (as extracted by `jd-analyzer`) and the user's `WorkAuthorization` as dealbreakers by default; the user **may** override per-job.

**Demographics and application autofill**
- `REQ-FUNC-DEMO-001` — The system **shall** store voluntary demographic self-identification data per the `Demographics` schema. All fields **shall** be optional; unset **shall** be indistinguishable externally from "prefer not to say." The UI **shall** display a clear explanatory notice on the Demographics panel describing purpose, use, and non-transmission guarantees.
- `REQ-FUNC-DEMO-002` — The system **shall** maintain a per-field `DemographicSharePolicy`. Default policy for every field on creation **shall** be `ask_each_time`. The user **shall** be able to change any field's policy at any time.
- `REQ-FUNC-DEMO-003` — The system **shall not** transmit demographic, work-authorization, or identity data to the LLM provider as free text. Skills that need to place such values in output **shall** emit named placeholders (e.g., `{{demographics.veteran_status}}`) that the backend fills in locally after LLM processing.
- `REQ-FUNC-DEMO-004` — When `application-autofiller` is invoked, fields with policy `never_share` **shall** be omitted; `always_share` fields **shall** be filled automatically; `share_on_request` and `ask_each_time` fields **shall** require explicit user confirmation per invocation before values are returned.
- `REQ-FUNC-DEMO-005` — Every invocation of `application-autofiller` **shall** write an `AutofillLog` entry recording exactly which fields were shared, with what values, under what policy, and the recipient (document id or "manual copy"). The UI **shall** surface a post-autofill summary showing the same information.
- `REQ-FUNC-DEMO-006` — The Demographics panel **shall** display a per-field "last shared" indicator so the user can see at a glance which fields have been emitted recently and to which jobs.
